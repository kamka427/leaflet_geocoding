var leafletControlGeocoder = (function (exports, L) {
  ...
  var AzureMapsGeocoder = /*#__PURE__*/function () {
    function AzureMapsGeocoder(options) {
    ...
    /**
     * Perform a forward geocoding request (address → coordinates)
     * @param query The address to geocode
     * @param cb Callback to handle the geocoding results
     * @param context Optional context for the callback
     */


    var _proto = AzureMapsGeocoder.prototype;
    ...
    _proto.geocode = function geocode(query, cb, context) {
      ...
      var url = this.options.serviceUrl + "/address/json?" + new URLSearchParams(params).toString();
        ...
        if (data.results && data.results.length > 0) {
          for (var _iterator = _createForOfIteratorHelperLoose(data.results), _step; !(_step = _iterator()).done;) {
            ...
            results.push({
              ...
              bbox: L__namespace.latLngBounds([result.viewport.topLeftPoint.lat, result.viewport.topLeftPoint.lon], [result.viewport.btmRightPoint.lat, result.viewport.btmRightPoint.lon]),
              center: L__namespace.latLng(result.position.lat, result.position.lon)
            });
          }
        }
    ...
    /**
     * Perform a reverse geocoding request (coordinates → address)
     * @param location Latitude and longitude
     * @param cb Callback to handle the reverse geocoding results
     * @param context Optional context for the callback
     */
    ;

    _proto.reverse = function reverse(location, cb, context) {
      var params = {
        'api-version': '1.0',
        query: location.lat + "," + location.lng,
        'subscription-key': this.options.apiKey
      };
      var url = this.options.serviceUrl + "/address/reverse/json?" + new URLSearchParams(params).toString();
      fetch(url).then(function (response) {
        return response.json();
      }).then(function (data) {
        var results = [];
        ...
        if (data.addresses && data.addresses.length > 0) {
          for (var _iterator2 = _createForOfIteratorHelperLoose(data.addresses), _step2; !(_step2 = _iterator2()).done;) {
            var address = _step2.value;
            results.push({
              name: address.address.freeformAddress,
              bbox: L__namespace.latLngBounds([address.viewport.topLeftPoint.lat, address.viewport.topLeftPoint.lon], [address.viewport.btmRightPoint.lat, address.viewport.btmRightPoint.lon]),
  ...
  /**
   * Factory function for AzureMapsGeocoder
  ...
  var geocoders = {
    ...
    Bing: Bing,
    bing: bing,
    AzureMapsGeocoder: AzureMapsGeocoder,
    azure: azure,
    Google: Google,
    google: google,
    HERE: HERE,
